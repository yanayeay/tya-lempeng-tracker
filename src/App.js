...
const PasswordProtection = ({ onAuthenticated }) => {
  ...
  // Default users - you can expand this list
  const defaultUsers = [
    { id: 1, username: 'Admin', password: 'TyaLempeng2024!', role: 'Administrator', active: true, createdDate: '2024-08-13' }
  ];
...
const FinanceTracker = ({ onLogout, currentUser }) => {
  ...
  // Initialize with sample data - YOUR EXISTING CATEGORIES
  const [incomeCategories, setIncomeCategories] = useState([
    'Direct Orkid', 'Direct Melur', 'Direct Cempaka', 'Asrap', 'Warung Zul','Gerai Kak Zura', 'Gerai Fafau'
  ]);

  const [expenseCategories, setExpenseCategories] = useState([
    'Ayien Withdraw', 'Ayien Own Expenses', 'Bawang Besar', 'Bawang Kecil',
    'Bawang Rose', 'Minyak', 'Tepung', 'Packaging Orkid', 'Container Sambal Orkid'
  ]);
  ...
  const [transactions, setTransactions] = useState([
    {
      ...
      quantity: 15,
      totalAmount: 675.00,
      category: 'Direct Orkid',
      description: 'Lempeng sales to regular customer',
      paymentMethod: 'cash',
      date: '2025-08-13',
    ...
    },
    {
      ...
      quantity: 2,
      totalAmount: 25.00,
      category: 'Bawang Besar',
      description: 'Ingredients for lempeng',
      paymentMethod: 'cash',
      date: '2025-08-12',
      createdAt: new Date().toISOString()
    },
  ...
  const [users, setUsers] = useState([
    { id: 1, username: 'Admin', password: 'TyaLempeng2024!', role: 'Administrator', active: true, createdDate: '2024-08-13', lastLogin: new Date().toISOString() }
  ]);
  ...
  const applyFilters = (transactionList) => {
    return transactionList.filter(transaction => {
      ...
      if (filters.paymentMethods.length > 0 && !filters.paymentMethods.includes(transaction.paymentMethod)) return false;
  ...
  const handleLogout = () => {
    if (window.confirm('Are you sure you want to logout? Your data will remain safe in this browser.')) {
      onLogout();
    }
  };
  ...
  const clearAllData = () => {
    if (window.confirm('Are you sure you want to clear all data? This cannot be undone.')) {
      setTransactions([]);
      setIncomeCategories(['Direct Orkid', 'Direct Melur', 'Direct Cempaka', 'Asrap', 'Warung Zul','Gerai Kak Zura', 'Gerai Fafau']);
  ...
  const importBackup = (event) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        if (backup.transactions && backup.incomeCategories && backup.expenseCategories) {
  ...
  const handleSubmit = () => {
    ...
    const transaction = {
      ...
      ...formData, amount: unitAmount, quantity: quantity, totalAmount: totalAmount,
  ...
  const handleEdit = (transaction) => {
    ...
    setFormData({
      ...
      paymentMethod: transaction.paymentMethod, date: transaction.date
    });
  ...
  const saveEditCategory = () => {
    ...
    if (editingCategory.type === 'income') {
      setIncomeCategories(prev => prev.map(cat => cat === oldCategory ? newCategoryName : cat));
    } else {
      setExpenseCategories(prev => prev.map(cat => cat === oldCategory ? newCategoryName : cat));
    }

    setTransactions(prev => prev.map(t => t.category === oldCategory ? { ...t, category: newCategoryName } : t));
  ...
  const handleUserSubmit = () => {
    ...
    const existingUser = users.find(u => u.username.toLowerCase() === userForm.username.toLowerCase() && u.id !== editingUser?.id);
    if (existingUser) {
    ...
    const userData = {
      ...
      ...userForm,
      createdDate: editingUser ? editingUser.createdDate : new Date().toISOString().split('T')[0],
  ...
  const calculateTotals = (transactionList = transactions) => {
    const income = transactionList.filter(t => t.type === 'income').reduce((sum, t) => sum + (t.totalAmount || t.amount), 0);
    const expenses = transactionList.filter(t => t.type === 'expense').reduce((sum, t) => sum + (t.totalAmount || t.amount), 0);
    const onlineTotal = transactionList.filter(t => t.paymentMethod === 'online').reduce((sum, t) => sum + (t.totalAmount || t.amount), 0);
    const cashTotal = transactionList.filter(t => t.paymentMethod === 'cash').reduce((sum, t) => sum + (t.totalAmount || t.amount), 0);
    const ayienSpending = transactionList.filter(t => t.category.toLowerCase().includes('ayien')).reduce((sum, t) => sum + (t.totalAmount || t.amount), 0);
  ...
  const exportToCSV = () => {
    ...
    const csvContent = [
      ...
      ...filteredTransactions.map(t => [
        ...
        t.amount, t.quantity || 1, t.totalAmount || t.amount, t.paymentMethod
      ].join(','))
    ].join('\n');
      ...
      <div className="bg-white rounded-lg shadow-sm p-6">
        ...
        <div className="bg-blue-50 p-3 rounded-lg mb-6 border border-blue-200">
          <p className="text-sm text-blue-700">
            ...
            <strong>Last Updated:</strong> {transactions.length > 0 ? new Date(Math.max(...transactions.map(t => new Date(t.createdAt)))).toLocaleString() : 'Never'}
          </p>
        </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-blue-600 mt-1">
                  {transactions.filter(t => t.paymentMethod === 'online').length} transactions
                </p>
              </div>
                ...
                <div key={transaction.id} className="flex items-center justify-between p-4 border rounded-lg hover:bg-gray-50">
                  ...
                  <div className="flex items-center gap-3">
                    <span className={`font-bold ${transaction.type === 'income' ? 'text-green-600' : 'text-red-600'}`}>
                      {transaction.type === 'income' ? '+' : '-'}RM {(transaction.totalAmount || transaction.amount).toFixed(2)}
                    </span>
